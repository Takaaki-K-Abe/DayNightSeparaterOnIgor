...
Function StartSunAlt(name)
	...
	DoWindow/K SelectLocationTypes
End
...
Function SetParametersForCalclateSolarAltitude()

	if(!DataFolderExists("root:DayNightSeparater"))
		NewDataFolder root:DayNightSeparater
	endif
	
	if(!DataFolderExists("root:DayNightSeparater:Parameters"))
		NewDataFolder root:DayNightSeparater:Parameters
	endif

	SetDataFolder root:DayNightSeparater:Parameters

	//! in the case for calculating one point
	variable/G Latitude
	variable/G Longitude
	//! the time difference in standard time from UTC time
	variable/G TimeDiffFromUTC
	//! the time interval for calculating solar altitude (seconds)
	variable/G IntervalOfTime
...
Function Calculate_SolarAltitude_From_Coordinate(ReferenceWave, TimeDiffFromUTC, Latitude, Longitude)
	...
	// start to compute the solar altitude
	Variable JulianDate
